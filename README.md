...
When `--input-root` points to a single scan folder (for example `.../5`), the script now automatically retries conversion against the parent study folder if the converter rejects the scan path.
