...
def _extract_roi_mask(label_img: nib.Nifti1Image, label_ids: list[int]) -> np.ndarray:
    data = np.asarray(label_img.get_fdata())
    return np.isin(data, np.array(label_ids, dtype=data.dtype)).astype(np.uint8)
...
def _run_one_subject(
    ...
    # Save final labels back in original raw voxel order and original affine.
...
def main() -> None:
    ...
    if not scans:
        raise SystemExit("No NIfTI scans found under input root. Convert Bruker to NIfTI first.")
